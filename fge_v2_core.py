...
# ==========================================
# 2. CORE ENGINES & PIPELINE SERVICES
# ==========================================
class LayerCollisionMatrix:
    def __init__(self):
        # Operational Garment Volume Index (GVI) Database
...
class RenderCompiler:
    ...
    def compile_weighted_prompt(fge_json: dict) -> str:
        ...
        # New Sub-Surface Cyberware Tokenization Layer
...
# ==========================================
# 3. END-TO-END PIPELINE RESOLUTION MANAGER
# ==========================================
class FashPipelineManager:
    ...
    def resolve_and_compile(self, inbound_data: dict, target_trend: Optional[str] = None) -> dict:
        # Step 1: Hot-swap trend overlay modifications if a swap is targeted
        active_config = inbound_data
        if target_trend:
            active_config = self.trend_injector.inject_trend(inbound_data, target_trend)
        
        # Step 2: Physical Mesh Collision Gatekeeping
        ...
        # Step 3: Run Render Module prompt math compiler
...
# ==========================================
# 4. FASTAPI GATEWAY ARCHITECTURE LAYOUT
# ==========================================
...
def resolve_fashion_pipeline(payload: FashModuleInbound, trend_override: Optional[str] = None):
    """
    Consumes inbound structured Fashion Objects, processes mutations/physics,
    and returns production-ready render prompt strings.
    """
    try:
        # Convert Pydantic object back into structural dict processing format
        inbound_dict = payload.model_dump()
        result = manager.resolve_and_compile(inbound_dict, target_trend=trend_override)
        return result
...
# ==========================================
# 5. AUTOMATED PIPELINE INTEGRATION UNIT TEST
# ==========================================
def run_automated_pipeline_unit_test():
    print("\n🚀 INITIALIZING AUTOMATED ASSET RESOLUTION PIPELINE UNIT TEST...")
    

# Inbound Sample payload mimicking deep character-attachment inputs
mock_input_payload = {
"character_id": "CH_HACKER_VAL",
"archetype": "Cyberpunk Hacker",
"silhouette": "Oversized Upper + Slim Lower",
# Including New Layer Type
...
"materials": {
"primary": "Distressed Leather",
"secondary": "Carbon Fiber"
},
...
# Instantiating a clean pipeline manager block
test_manager = FashPipelineManager()
try:
# Action: Process baseline payload through collision matrix and compiler
print("\n[TEST CASE 1]: Running Base Archetype Engine...")
...
print(f" -> {base_output['render_prompt'][:120]}...")
# Action: Inject alternative trend (Neo-Gothic) to execute asset mutations & check structural scaling
print("\n[TEST CASE 2]: Injecting Detachable Neo-Gothic Trend Overlay Matrix...")
mutated_output = test_manager.resolve_and_compile(mock_input_payload, target_trend="Neo-Gothic")
# Verify sub-surface cyberware layer persists even when outer gear swaps styles
assert len(mutated_output["resolved_configuration"]["sub_surface_cyberware"]) == 2
# Verify layer garments structural swapping occurred cleanly
...
print(f" -> {mutated_output['render_prompt'][:150]}...")
# Action: Intentionally cause a Layer Collision error to test automated gatekeeping safety mechanics
print("\n[TEST CASE 3]: Simulating Visual Layer Mesh Collision Fault...")
broken_payload = mock_input_payload.copy()
broken_payload["base_layer"] = ["Oversized Cashmere Trench Coat"] # Volume 5 inside a volume 3 jacket
try:
...
print(f"❌ TEST CRITICAL INTEGRITY EXCEPTION: {str(assert_err)}")
